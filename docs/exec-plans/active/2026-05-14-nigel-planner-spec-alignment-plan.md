...
The `>=` guard is defense-in-depth, NOT the primary idempotency mechanism. The actual single-execution guarantee comes from `assertValidTransition` in `state-machine.ts`: terminal states (`completed` / `failed` / `cancelled`) have empty outgoing transition sets, and identity transitions are explicitly excluded. So a second `updateRunStatus(childRunId, "completed")` throws `InvalidTransitionError` before the release SQL ever runs.

Workflow SDK replay safety is therefore at the *step* boundary, not the SQL boundary: callers should follow the existing pattern in `linear-trigger.ts:48-67` (check current status before calling `updateRunStatus` and short-circuit on already-terminal), so a replayed step records success without re-executing the SQL. The `>=` guard exists to protect against a future code path that bypasses `updateRunStatus` and calls the release SQL directly — without it, such a bypass could underflow `cost_usd_reserved_micros`. It is not, and should not be presented as, the layer that makes "terminal-status update called twice" safe.
...
- Idempotency of terminal release at the state-machine layer: dispatch one child with `budget = $2`, mark it `completed` (succeeds, `costUsdReservedMicros` decremented by $2). Call `updateRunStatus(childRunId, "completed")` a second time; expect `InvalidTransitionError` from `assertValidTransition` (terminal states have empty outgoing transitions). Verify the release SQL did NOT execute twice — `costUsdReservedMicros` is still decremented by exactly $2, not $4. This is the primary single-execution guarantee.
- Workflow SDK replay short-circuit at the step layer: dispatch one child, mark it `completed`. Re-invoke the step function that would call `updateRunStatus` (simulating a Workflow SDK kill-9 replay where the step re-runs because completion was not yet recorded). Expect the step to short-circuit at the `current.status === "completed"` check (mirroring the `markRunRunning` pattern in `linear-trigger.ts:48-67`) and return success without calling `updateRunStatus` a second time.
- `>=` guard as defense-in-depth: bypass `updateRunStatus` entirely and call the release SQL directly twice against a root with `costUsdReservedMicros = $2`. First call decrements to $0 (succeeds). Second call's `WHERE cost_usd_reserved_micros >= 2` evaluates to false, the SQL is a no-op, `costUsdReservedMicros` stays at $0 — no underflow. This guard is the safety net if a future code path bypasses the state-machine layer.
